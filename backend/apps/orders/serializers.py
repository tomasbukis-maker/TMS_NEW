...
class CargoItemSerializer(serializers.ModelSerializer):
    ...
    loading_stop_info = serializers.SerializerMethodField()
    unloading_stop_info = serializers.SerializerMethodField()
    ...
    class Meta:
        ...
        fields = [
            ...
            'notes', 'created_at', 'loading_stop', 'unloading_stop',
            'loading_stop_info', 'unloading_stop_info'
        ]
        read_only_fields = ['id', 'created_at']

    def get_loading_stop_info(self, obj):
        if obj.loading_stop:
            return {
                'id': obj.loading_stop.id,
                'city': obj.loading_stop.city,
                'country': obj.loading_stop.country,
                'name': obj.loading_stop.name
            }
        return None

    def get_unloading_stop_info(self, obj):
        if obj.unloading_stop:
            return {
                'id': obj.unloading_stop.id,
                'city': obj.unloading_stop.city,
                'country': obj.unloading_stop.country,
                'name': obj.unloading_stop.name
            }
        return None
...
class OrderCarrierSerializer(serializers.ModelSerializer):
    ...
    effective_route_from = serializers.SerializerMethodField()
    effective_route_to = serializers.SerializerMethodField()
    effective_route_from_country = serializers.SerializerMethodField()
    effective_route_from_city = serializers.SerializerMethodField()
    effective_route_from_address = serializers.SerializerMethodField()
    effective_route_to_country = serializers.SerializerMethodField()
    effective_route_to_city = serializers.SerializerMethodField()
    effective_route_to_address = serializers.SerializerMethodField()
    effective_loading_date_from = serializers.SerializerMethodField()
    effective_loading_date_to = serializers.SerializerMethodField()
    effective_unloading_date_from = serializers.SerializerMethodField()
    effective_unloading_date_to = serializers.SerializerMethodField()
    ...
    # ===== EFEKTYVŪS DUOMENŲ GET METODAI =====

    def get_effective_route_from(self, obj):
        return obj.effective_route_from

    def get_effective_route_to(self, obj):
        return obj.effective_route_to

    def get_effective_route_from_country(self, obj):
        return obj.effective_route_from_country

    def get_effective_route_from_city(self, obj):
        return obj.effective_route_from_city

    def get_effective_route_from_address(self, obj):
        return obj.effective_route_from_address

    def get_effective_route_to_country(self, obj):
        return obj.effective_route_to_country

    def get_effective_route_to_city(self, obj):
        return obj.effective_route_to_city

    def get_effective_route_to_address(self, obj):
        return obj.effective_route_to_address

    def get_effective_loading_date_from(self, obj):
        return obj.effective_loading_date_from

    def get_effective_loading_date_to(self, obj):
        return obj.effective_loading_date_to

    def get_effective_unloading_date_from(self, obj):
        return obj.effective_unloading_date_from

    def get_effective_unloading_date_to(self, obj):
        return obj.effective_unloading_date_to
    ...
    def validate(self, attrs):
        ...
        if expedition_number:
            ...
            # Tikrinti unikalumą tik tame pačiame užsakyme
            order_id = attrs.get('order_id') or attrs.get('order')
            if order_id:
                if isinstance(order_id, int):
                    order_pk = order_id
                elif hasattr(order_id, 'pk'):
                    order_pk = order_id.pk
                else:
                    order_pk = order_id

                qs = OrderCarrier.objects.filter(
                ...
                if qs.exists():
                    raise serializers.ValidationError({'expedition_number': 'Šis ekspedicijos numeris jau naudojamas šiame užsakyme.'})
            else:
                # Jei nėra order_id, naudoti seną logiką (turėtų būti reta)
    ...
    def _maybe_generate_expedition_number(self, carrier_type: str, current_value: Optional[str]) -> Optional[str]:
        if carrier_type not in [OrderCarrier.CarrierType.CARRIER, OrderCarrier.CarrierType.WAREHOUSE]:
            return None
        if current_value:
        ...
        try:
            ...
            expedition_type = 'warehouse' if carrier_type == OrderCarrier.CarrierType.WAREHOUSE else 'carrier'
            return generate_expedition_number(expedition_type=expedition_type)
    ...
    class Meta:
        ...
        fields = [
            ...
            # Seni laukai (atgalinio suderinamumo dėlei)
            'route_from', 'route_to',
            'route_from_country', 'route_from_postal_code', 'route_from_city', 'route_from_address',
            'sender_name',
            'route_to_country', 'route_to_postal_code', 'route_to_city', 'route_to_address',
            'receiver_name', 'loading_date', 'unloading_date',
            'loading_date_from', 'loading_date_to', 'unloading_date_from', 'unloading_date_to',
            # Nauji efektyvūs laukai
            'effective_route_from', 'effective_route_to',
            'effective_route_from_country', 'effective_route_from_city', 'effective_route_from_address',
            'effective_route_to_country', 'effective_route_to_city', 'effective_route_to_address',
            'effective_loading_date_from', 'effective_loading_date_to',
            'effective_unloading_date_from', 'effective_unloading_date_to',
            # Vėliavėlės
            'has_custom_route', 'has_custom_dates',
...
class OrderCostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = OrderCost
        fields = [
            'id', 'order', 'order_id', 'partner', 'partner_id', 'cost_type', 'cost_type_display',
            'expedition_number', 'amount_net', 'vat_rate', 'vat_amount', 'amount_with_vat',
            'invoice_issued', 'invoice_received', 'invoice_number', 'invoice_date',
            'payment_status', 'payment_status_display', 'payment_date', 'due_date',
            'payment_terms', 'description', 'status', 'status_display', 'sequence_order',
            'created_at', 'updated_at', 'days_overdue', 'is_overdue'
        ]
        read_only_fields = ['id', 'created_at', 'updated_at', 'expedition_number']
...
class RouteStopSerializer(serializers.ModelSerializer):
    """Maršruto sustojimų serializer"""
    stop_type_display = serializers.CharField(source='get_stop_type_display', read_only=True)
    
    class Meta:
        model = RouteStop
        fields = [
            'id', 'order', 'stop_type', 'stop_type_display', 'sequence_order',
            'name', 'country', 'postal_code', 'city', 'address',
...
class OrderSerializer(serializers.ModelSerializer):
    ...
    payment_status_info = serializers.SerializerMethodField()
    route_type_display = serializers.SerializerMethodField()
    ...
    def get_route_type_display(self, obj):
        """Get human-readable route type display"""
        try:
            return obj.get_route_type_display()
        except AttributeError:
            return getattr(obj, 'route_type', 'simple')
    ...
    def get_route_stops(self, obj):
        """Serializuoja route_stops, bet tik jei instance turi PK"""
        if obj.pk is None:
            return []
    ...
    def get_costs(self, obj):
        """Serializuoja order costs"""
        try:
            # Jei instance dar neturi PK, negalime pasiekti related objects
            if obj.pk is None:
                return []
            if not hasattr(obj, 'costs'):
                return []
            costs = obj.costs.all()
            if not costs:
                return []
            # Serializuoti visus costs vienu kartu
            serializer = OrderCostSerializer(costs, many=True, context=self.context)
            return serializer.data
        except (AttributeError, Exception) as e:
            print(f"Error serializing costs: {e}")
            import traceback
            traceback.print_exc()
            return []
    ...
    class Meta:
        model = Order
        fields = [
            'id', 'order_number', 'client_order_number', 'client', 'client_id', 'order_type', 'order_type_display',
            ...
            'requires_forklift', 'requires_crane', 'requires_special_equipment',
            'fragile', 'hazardous', 'temperature_controlled', 'requires_permit',
            'notes', 'created_at', 'updated_at', 'created_by', 'carriers', 'cargo_items', 'route_stops', 'costs',
    ...
    def _save_autocomplete_suggestions(self, instance, validated_data):
        ...
        try:
            ...
            def normalize_value(value, field_type):
                """Normalizuoja reikšmę prieš išsaugojimą"""
                if not value:
                    return value
                ...
                if field_type in ['address', 'order_notes', 'cargo_description']:
                    # Adresams ir pastaboms - pašalinti nereikalingus simbolius pabaigoje
                    normalized = re.sub(r'[.,;]+$', '', normalized.strip())
                    # Normalizuoti "g." į "g.", "pl." į "pl." ir t.t.
                    ...
                    # Miestams - padaryti title case ir pašalinti nereikalingus simbolius
                    normalized = normalized.title()
                    normalized = re.sub(r'[.,;]+$', '', normalized.strip())

                elif field_type == 'country':
                    # Šalims - padaryti title case
                    normalized = normalized.title()
                    # Žinomi šalies pavadinimų mapping'ai
                    country_mappings = {
                        'Lt': 'Lietuva',
                        'Lt.': 'Lietuva',
                        'Lithuania': 'Lietuva',
            ...
            def save_suggestion(field_type, value):
                if value and value.strip():
                    try:
                        # Normalizuojame reikšmę prieš išsaugojimą
                        ...
                        if not created:
                            suggestion.usage_count += 1
                            suggestion.save()
                        logger.debug(f"Saved autocomplete suggestion: {field_type} = {normalized_value}")
                    except Exception as e:
